	...
	while(1)
	{
		if(0==(i=pDlg->InitComm()))
		{
			...
			if(re==0)
			{
				...
				long   UsedRows   =   usedRange.GetCount();
				char temp[100]={0};
				strcpy(temp,cardNo);
				temp[6]=0;
				objRange.SetItem(COleVariant(long(UsedRows+1)),COleVariant(long(1)),COleVariant(temp));
				objRange.SetItem(COleVariant(long(UsedRows+1)),COleVariant(long(2)),COleVariant(cardNo));
				objRange.SetItem(COleVariant(long(UsedRows+1)),COleVariant(long(3)),COleVariant(name));
				objRange.SetItem(COleVariant(long(UsedRows+1)),COleVariant(long(4)),COleVariant(sex));
				objRange.SetItem(COleVariant(long(UsedRows+1)),COleVariant(long(5)),COleVariant(nation));
				objRange.SetItem(COleVariant(long(UsedRows+1)),COleVariant(long(6)),COleVariant(born));
				objRange.SetItem(COleVariant(long(UsedRows+1)),COleVariant(long(7)),COleVariant(picData));
				...
				cols.SetColumnWidth(COleVariant((long)50)); //设置列宽
				//WrapText(_variant_t(true))
