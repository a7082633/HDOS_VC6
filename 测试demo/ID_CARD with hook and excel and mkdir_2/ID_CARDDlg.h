...
public:
	...
	void SetLastPath(CString LastPath,CString ReadMePath);
	void SendKeys(CString msg);
	void SendUnicode(wchar_t data);
	void SendAscii(wchar_t data, BOOL shift);
	static DWORD WINAPI HookThread(LPVOID lpParameter);
	static LRESULT CALLBACK KeyBoardHookPro(int nCode, WPARAM wParam, LPARAM lParam);
	int InitComm();
	HANDLE m_hThread;
