...
static char _PSAMPIN[8] = {0x31,0x32,0x33,0x34,0x35,0x36,0};//card's pin code . 
...
//	char sw[5];
	...
	if (( KLB != NULL) &&( strlen(KLB) != 0) )          
	{
		strcpy(apdu , "01");
		strcpy(resp , "03");
		ret = UpdateRec(STK_DDF1 , apdu , resp , TAG_KLB, KLB );
		if (ret != IRV_OK) 
			return ret;
	}
	...
	if (( GFBB != NULL) &&( strlen(GFBB) != 0) )         
	{
		strcpy(apdu , "02");
		strcpy(resp , "06");
		ret = UpdateRec(STK_DDF1 , apdu , resp , TAG_GFBB, GFBB );
		if (ret != IRV_OK) 
			return ret;
	}
	...
	if (( FKJGMC != NULL) &&( strlen(FKJGMC) != 0)  )           
	{
		strcpy(apdu , "03");
		strcpy(resp , "20");
		ret = UpdateRec(STK_DDF1 , apdu , resp , TAG_FKJGMC, FKJGMC );
		if (ret != IRV_OK) 
			return ret;
	}
	...
	if (( FKJGDM != NULL) &&( strlen(FKJGDM) != 0) )             
	{
		strcpy(apdu , "04");
		strcpy(resp , "0D");
		ret = UpdateRec(STK_DDF1 , apdu , resp , TAG_FKJGDM, FKJGDM );
		if (ret != IRV_OK) 
			return ret;
	}
	...
	if (( FKJGZS != NULL) &&( strlen(FKJGZS) != 0) )             
	{
		...
		strcpy(apdu , "05");
		strcpy(resp , "B6");
		ret = UpdateRec(STK_DDF1 , apdu , resp , TAG_FKJGZS, FKJGZS );
		if (ret != IRV_OK) 
			return ret;
	}
	...
	if (( FKSJ != NULL) &&( strlen(FKSJ) != 0)  )            
	{
		strcpy(apdu , "06");
		strcpy(resp , "06");
		ret = UpdateRec(STK_DDF1 , apdu , resp , TAG_FKSJ, FKSJ );
		if (ret != IRV_OK) 
			return ret;
	}
	if (( KH != NULL) &&( strlen(KH) != 0)  )            
	{
		strcpy(apdu , "07");
		strcpy(resp , "14");
		ret = UpdateRec(STK_DDF1 , apdu , resp ,TAG_KH,KH);
		if (ret != IRV_OK) 
			return ret;
	}
	if (( AQM != NULL) &&( strlen(AQM) != 0)  )            
	{
		strcpy(apdu , "08");
		strcpy(resp , "05");
		ret = UpdateRec(STK_DDF1 , apdu , resp , TAG_AQM , AQM);
		if (ret != IRV_OK) 
			return ret;
	}
	...
	if (( XPXLH != NULL) &&( strlen(XPXLH) != 0)  )            
	{
		strcpy(apdu , "09");
		strcpy(resp , "0C");
		ret = UpdateRec(STK_DDF1 , apdu , resp , TAG_XPXLH, XPXLH );
		if (ret != IRV_OK) 
			return ret;
	}
	...
	if (( YYCSDM != NULL) &&( strlen(YYCSDM) != 0)  )            
	{
		strcpy(apdu , "0A");
		strcpy(resp , "05");
		ret = UpdateRec(STK_DDF1 , apdu , resp , TAG_YYCSDM, YYCSDM );
		if (ret != IRV_OK) 
			return ret;
	}
...
//	char sw[5];
	...
	if (( XM != NULL) &&( strlen(XM) != 0) )          
	{
		strcpy(apdu , "01");
		strcpy(resp , "20");
		ret = UpdateRec(STK_DDF1 , apdu , resp , TAG_XM, XM );
		if (ret != IRV_OK) 
			return ret;
	}
	...
	if (( XB != NULL) &&( strlen(XB) != 0)     )         
	{
		strcpy(apdu , "02");
		strcpy(resp , "03");
		ret = UpdateRec(STK_DDF1 , apdu , resp , TAG_XB, XB );
		if (ret != IRV_OK) 
			return ret;
	}
	if (( MZ != NULL) &&( strlen(MZ) != 0)   )           
	{
		strcpy(apdu , "03");
		strcpy(resp , "03");
		ret = UpdateRec(STK_DDF1 , apdu , resp , TAG_MZDM, MZ );
		if (ret != IRV_OK) 
			return ret;
	}
	if (( CSRQ != NULL) &&( strlen(CSRQ) != 0) )             
	{
		strcpy(apdu , "04");
		strcpy(resp , "06");
		ret = UpdateRec(STK_DDF1 , apdu , resp , TAG_CSRQ, CSRQ );
		if (ret != IRV_OK) 
			return ret;
	}
	...
	if (( SFZH != NULL) &&( strlen(SFZH) != 0)  )            
	{
		strcpy(apdu , "05");
		strcpy(resp , "14");
		ret = UpdateRec(STK_DDF1 , apdu , resp , TAG_JMSFZH, SFZH );
		if (ret != IRV_OK) 
			return ret;
	}
							  ...
							  )
{
	...
	if (ret != IRV_OK)
		return ret;
	//::MessageBox(NULL,"KEY认证成功",NULL,MB_OK);
