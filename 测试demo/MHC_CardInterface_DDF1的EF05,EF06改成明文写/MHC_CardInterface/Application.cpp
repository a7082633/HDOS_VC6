...
int UpdateRec(IN int keyID ,IN char * RecID,IN char* Lc, IN int datatag,  IN char *LcData )
{
	char apdu[512 + 10 + 1];
	char resp[256];
	char apduHead[11];
	...
	//prepare data
	datatoTLV(datatag , LcData , resp);
	strcpy(apduHead , "00DC");
	strcat(apduHead , RecID );
	strcat(apduHead , "04");	
	strcat(apduHead , Lc);
	sprintf(apdu,"%s%s",apduHead,resp);
	//strcat(apdu,apduHead);
	char temp[1000]={0};
//	sprintf(temp,"准备发送指令%s",apdu);
//	::MessageBox(NULL,temp,NULL,MB_OK);
	int ret = rd.RF_Send(apdu , resp , sw);
	//sprintf(temp,"返回%s",sw);
	//::MessageBox(NULL,temp,NULL,MB_OK);
	if ((ret != IRV_OK) ||(strcmp(sw , "9000") != 0))
		return IRV_WR_RCDERR;
