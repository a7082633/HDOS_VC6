...
{
	::MessageBox(NULL,"打开端口",NULL,MB_OK);
	HANDLE hDev=NULL;
...
BHGX_HD_API int __stdcall ICCSet( unsigned int iCardSet, unsigned char *pCardType, unsigned char *pRet )
{
	...
	if(iCardSet == 0)
	{
		...
		if(re != 0)
		{
			::MessageBox(NULL,"设置TypeA",NULL,MB_OK);
			return -38;
		}
		re = PICC_Reader_Request(DevHandle);
		if(re != 0)
		{
			::MessageBox(NULL,"请求卡片",NULL,MB_OK);
			return -38;
		}
		re = PICC_Reader_anticoll(DevHandle, uid);
		if(re != 0)
		{
			::MessageBox(NULL,"防碰撞",NULL,MB_OK);
			return -38;
		}
		...
		if(re < 0)
		{
			...
			if(re != 0)
			{
				::MessageBox(NULL,"M1请求",NULL,MB_OK);
				return -38;
			}
			re = PICC_Reader_anticoll(DevHandle, uid);
			if(re != 0)
			{
				::MessageBox(NULL,"M1防碰撞",NULL,MB_OK);
				return -38;
			}
			...
			if(re != 0)
			{
				::MessageBox(NULL,"M1选卡",NULL,MB_OK);
				return -38;
			}
