...
int __stdcall IC_GetData(ICC_ENV *iccenv,int datatype,char *data)
{
	...
	memset(cmd,0,100);
	memset(ResponseAPDU,0,200);
	memcpy(cmd, "\x00\xB2\x01\x04",4);
	len = ICC_Reader_Application(hReader,0x01,4,cmd,ResponseAPDU);
	if(len<=0 || ResponseAPDU[len-2]!=0x90)
	{
		char *p="00B201";
		memcpy(data,p,strlen(p));
	...
	char IdNo[0x13]={0};
	memcpy(IdNo,ResponseAPDU+2,0x12);//获身份证
	//读姓名
	memset(cmd,0,100);
	memset(ResponseAPDU,0,200);
	memcpy(cmd, "\x00\xB2\x02\x04",4);
	len = ICC_Reader_Application(hReader,0x01,4,cmd,ResponseAPDU);
	if(len<=0 || ResponseAPDU[len-2]!=0x90)
	{
		char *p="00B202";
	...
	char Name[0x1f]={0};
	memcpy(Name,ResponseAPDU+2,0x1e);//获姓名
	//------------------------------------------------------------------------------
	//选DDF03
	memset(cmd,0,100);
	memset(ResponseAPDU,0,200);
	...
	memset(cmd,0,100);
	memset(ResponseAPDU,0,200);
	memcpy(cmd, "\x00\x20\x00\x00\x03\x12\x34\x57",8);
	len = ICC_Reader_Application(hReader,0x01,8,cmd,ResponseAPDU);
	if(len<=0 || ResponseAPDU[len-2]!=0x90)
	{
		memset(cmd,0,100);
		memset(ResponseAPDU,0,200);
		ICC_DispSound(hReader,4,0);
		unsigned char len=0;
		unsigned char pass[100]={0};
		int n=ICC_GetInputPass(hReader,100,&len,0,1,pass);
		memcpy(cmd, "\x00\x20\x00\x00",4);
		cmd[4]=len;
		memcpy(cmd+5,pass,len);
		len = ICC_Reader_Application(hReader,0x01,5+len,cmd,ResponseAPDU);
		if(len<=0 || ResponseAPDU[len-2]!=0x90)
		{
			char *p="VeryfyPin-002000";
			memcpy(data,p,strlen(p));
			return 4;
		}
	}
...
//	ich=ich+1+0x10*2;
	...
	memcpy(data+ich+1,IdNo,0x12);
	data[ich+1+0x12]=ch;
	ich=ich+1+0x12;

	memcpy(data+ich+1,Name,0x1e);
	//data[ich+1+0x1e]=ch;
	ich=ich+1+0x12;
