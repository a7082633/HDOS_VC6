...
{/*
	...
	int RespLen=0;
	int re=ICC_Reader_Reset(Reader::m_hReader,0,Response,&RespLen);
	if(re) return re;
//	int re=0;
//	unsigned char Response[128]={0};
...
//		return RD_NO_ICC;
//	}
	u.BinToHexstr((unsigned char*)_ATS_Responce , Response , RespLen);
	return RD_OK;
}
...
/*
	...
	int RespLen=0;
	retval = ICC_Reader_Application(Reader::m_hReader,0,len,(char *)send,(char *)resp,&RespLen);
	//retval = PICC_Reader_Application(Reader::m_hReader,len , send  ,resp);
	if (retval<0)
		return RD_IC_NO_RESP;

	u.BinToHexstr((unsigned char*)_APDU_Responce , resp , RespLen - 2) ;
	u.BinToHexstr((unsigned char*)_SW , resp + RespLen - 2 , 2);
...
int Reader::PSAM_Reset(int slot , char *_ATS_Responce)
{
	...
	int RespLen=0;
	retval = ICC_Reader_Reset(Reader::m_hReader,_SLOT, resp,&RespLen);
	//retval = ICC_Reader_PowerOn(Reader::m_hReader,iLayout , (unsigned char*)resp );
	if (retval<0) 
		return RD_NO_ICC;   

	u.BinToHexstr((unsigned char*)_ATS_Responce , resp , (int)RespLen);
...
int Reader::PSAM_Send( char * _APDU_CMD, char *_APDU_Responce ,  char * _SW)
{
	...
	int RespLen=0;
	retval = ICC_Reader_Application(Reader::m_hReader,2,len,(char *)send,(char *)resp,&RespLen);
...
int Reader::PSAM_Send2(char *_APDU_CMD, char *_APDU_Responce, char *_SW)
{
	int retval;
	unsigned char len = (unsigned char) (strlen((char *)_APDU_CMD) / 2);
	unsigned char send[260];
	...
	int RespLen=0;
	retval = ICC_Reader_Application(Reader::m_hReader,1,len,(char *)send,(char *)resp,&RespLen);
	//retval = ICC_Reader_Application(Reader::m_hReader,_SLOT,len , send  ,resp);
	if (retval<2)
		return RD_IC_NO_RESP;
