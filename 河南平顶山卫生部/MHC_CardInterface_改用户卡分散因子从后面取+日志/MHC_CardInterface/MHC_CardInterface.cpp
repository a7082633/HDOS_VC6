...
MHC_CARDINTERFACE_API int __stdcall PowerOn(HANDLE hDev,int slot, char * ATR) 
{
	...
	if (slot == SLOT_USER) 
	{
		...
		if (ret == IRV_OK)
		{
			...
			return ret;
		//选MF
		strcpy((char *)apdu , "00A40000023F00");
							  ...
							  )//全写
{
	...
	for(int i=0;i<len;i++)
	{
		...
		if(s<0)//最后一段/唯一的一段
		{
			...
			{
				int len=strlen((const char *)temp);
				char *p=NULL;
				p=strchr((const char *)temp,'.');
				if(p)
				{
					*p='0';
					while(*(p+1)!=0)
				...
				if(strlen((const char *)temp)%2!=0)
				{
					temp[strlen((const char *)temp)]='F';
				}
	...
	for(int i=0;i<len;i++)
	{
		...
		if(s<0)//最后一段/唯一的一段
		{
			...
			if(_stDataFileMAP[i+213].cCoding==CN_style)
			{
				...
				memset(tempdata+post,0xff,_stDataFileMAP[i+213].iDataLen);
				u.HexstrToBin(tempdata+post , temp , strlen((const char *)temp));
...
//	int len = strlen(szData);
	...
	char temp[400]={0};
//	char filter[400]={0};
//	memset(filter,0x30,399);
//	char scmp[400]={0};  //用于比较
...
//				*p='.';
			...
			strcpy(temp,s.c_str());
			strcat(szData,temp);
//			strncpy(scmp,filter,_stDataFileMAP[i+213].iDataLen*2);
//			if(strcmp(scmp,temp)!=0)
//				strcat(szData,temp);
//			else
//				strcat(szData,"");
		}
