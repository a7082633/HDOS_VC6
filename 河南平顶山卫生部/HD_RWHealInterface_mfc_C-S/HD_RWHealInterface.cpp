...
int __stdcall IDClosePort(void)
{
	return iDClosePort();
}
int __stdcall IVerifyPIN(HANDLE hDev,char * PIN)
{
	int rt=iVerifyPIN(hDev,PIN);
	return rt;
}
//int __stdcall IPowerOn(HANDLE hDev,int slot, 
...
MHC_CARDINTERFACE_API int __stdcall ReadCard(char *para,char *dataOut,
												char *account,char *password,
												int doctype,char *userid,
												char *pin)
{
	//::MessageBox(NULL,"开始检测程序","提示",MB_OK);
	//读之前判断进程是否存在
	if(!FindProcess())
	{
...
//	if(rt!=0) return rt;
	...
	FXN4 ICC_Reader_PowerOff=(FXN4)::GetProcAddress(hModule,"ICC_Reader_PowerOff"); //非触卡下电
	...
	//取卡的芯片号
		//请求卡片
	int rt=PICC_Reader_Request(hReader);
	if(rt!=0)
	{
...
MHC_CARDINTERFACE_API int __stdcall WriteCard(char *para,char *dataIn,
												char *account,char *password,
												int doctype,char *userid,
												char *pin)
{
...
//	HMODULE hModule=::LoadLibrary(configFullPath);
	...
	FXN6 ICC_Reader_GetDeviceCSN=(FXN6)::GetProcAddress(hModule,"ICC_Reader_GetDeviceCSN"); //获取芯片号
	FXN7 ICC_Reader_PowerOn=(FXN7)::GetProcAddress(hModule,"ICC_Reader_PowerOn"); //非接上电
	FXN8 ICC_Reader_Application=(FXN8)::GetProcAddress(hModule,"ICC_Reader_Application"); //发送指令
	//写之前需要验证
	long hReader=ICC_Reader_Open("USB1");
	char cmd[100]={0};
