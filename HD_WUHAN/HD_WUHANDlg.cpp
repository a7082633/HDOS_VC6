...
BEGIN_MESSAGE_MAP(CHD_WUHANDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BTN_READOTHER, OnBtnReadother)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
DWORD WINAPI CHD_WUHANDlg::ReaderThread(LPVOID lpParameter)
{
	...
	{
		char dataOut[500]={0};
		int i=IC_GetMagneData(dataOut,5);
		::Sleep(100);
		//pDlg->MessageBox("OK");
		if(0==i)
		{
			pDlg->SetDlgItemText(IDC_EDIT_DATA,dataOut);
		}
...
void CHD_WUHANDlg::OnBtnReadother() 
{
	char dataOut[500]={0};
	char pErr[50]={0};
	CString time="0";
