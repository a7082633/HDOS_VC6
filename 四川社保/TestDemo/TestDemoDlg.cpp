...
void CTestDemoDlg::OnBtnCall() 
{
	char data[1000]={0};
	char flag[100]={0};
	char err[100]={0};
	CString sflag,sData;
	this->GetDlgItemText(IDC_EDIT_INPUT,sflag);
	this->GetDlgItemText(IDC_EDIT_DATA,sData);
//	this->MessageBox(sflag);
	sprintf(data,"%s",sData.GetBuffer(0));
	int r=Run(sflag.GetBuffer(0),data,err);
	if(r==0)
		this->SetDlgItemText(IDC_EDIT_DATAFIELD,data);
	else
	{
		this->SetDlgItemText(IDC_EDIT_DATAFIELD,err);
	}
	sflag.ReleaseBuffer();
	sData.ReleaseBuffer();
}
