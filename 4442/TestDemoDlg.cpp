...
void CTestDemoDlg::OnBtnElectr() 
{
	...
	FXN4 PowerOn=(FXN4)::GetProcAddress(hModule,"ICC_Reader_4442_PowerOn");//上电
...
{
	FXN5 PowerOff=(FXN5)::GetProcAddress(hModule,"ICC_Reader_4442_PowerOff");//下电
...
{
	FXN6 Read=(FXN6)::GetProcAddress(hModule,"ICC_Reader_4442_Read");//读数据
	unsigned  char data[256]={0};
	CString start;
	CString size;
...
{
	FXN7 Write=(FXN7)::GetProcAddress(hModule,"ICC_Reader_4442_WriteHEX");//写数据
	unsigned  char data[256]={0};
	CString start;
	CString size;
	CString sData;
...
{
	FXN8 Login=(FXN8)::GetProcAddress(hModule,"ICC_Reader_4442_VerifyHEX");//登录
	CString sPass;
...
{
	FXN9 ReadCount=(FXN9)::GetProcAddress(hModule,"ICC_Reader_4442_ReadCount");//获取剩余次数
